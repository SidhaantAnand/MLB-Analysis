-- Win ratio per team
WITH numWins AS ( SELECT count(*) as wins,team_id FROM GameTeamStats WHERE won = 'W' GROUP BY team_id),
...
-- win ratio vs each team
WITH filterTeam AS (SELECT * FROM GameTeamStats WHERE team_id = 'was'),
noFilterTeam AS ( SELECT * FROM GameTeamStats WHERE team_id != 'was'),
...
-- Home record
WITH homeOnly AS ( SELECT * FROM GameTeamStats WHERE is_home_team = 1),
...
-- top 5  batter (in terms of hits) 
WITH hitsFilter AS ( SELECT batter_id, event, g_id FROM AtBats WHERE event = 'Home Run' OR event = 'Single' OR event = 'Double' ),
teamFilter AS ( SELECT * FROM GameTeamStats WHERE team_id = 'was'),
...
-- top 5  batter (in terms of number of homeruns) 
WITH hitsFilter AS ( SELECT batter_id, event, g_id FROM AtBats WHERE event = 'Home Run'),
teamFilter AS ( SELECT * FROM GameTeamStats WHERE team_id = 'was'),
teamHits AS ( SELECT batter_id FROM teamFilter INNER JOIN hitsFilter USING (g_id)),
countHits AS ( SELECT COUNT(*) AS num_hits,batter_id FROM teamHits GROUP BY (batter_id) ),
...
-- top 5 pitchers ( in terms of strikeouts ratio)
WITH pitchesFilter AS ( SELECT pitcher_id, event, g_id FROM AtBats WHERE event = 'Strikeout' ),
teamFilter AS ( SELECT * FROM GameTeamStats WHERE team_id = 'was'),
...
-- Most Player appearences for a team

WITH batters AS ( SELECT distinct g_id, batter_id FROM AtBats WHERE g_id IN (SELECT distinct g_id FROM GameTeamStats WHERE team_id = 'was')),
pitchers AS ( SELECT distinct g_id,pitcher_id FROM AtBats WHERE g_id IN (SELECT distinct g_id FROM GameTeamStats WHERE team_id = 'was')),
