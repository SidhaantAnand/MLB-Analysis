...
create table Pitches (
	...
	px decimal(2, 2),
	pz decimal(2, 2),
	break_angle decimal(3),
	break_length decimal(6, 3),
	break_y decimal(2, 2),
	ax decimal(2, 2),
	ay decimal(2, 2),
	az decimal(2, 2),
...
load data infile '/var/lib/mysql-files/MLB/pitches.csv' ignore into table Pitches
	...
	set
		...
		break_angle = if (@break_angle like '', NULL, @break_angle),
		break_length = if (@break_length like '', NULL, @break_length),
		break_y = if (@break_y like '', NULL, @break_y),
		ax = if (@ax like '', NULL, @ax),
		ay = if (@ay like '', NULL, @ay),
		az = if (@az like '', NULL, @az),
		sz_top = if (@sz_top like '', NULL, @sz_top),
		vx0 = if (@vx0 like '', NULL, @vx0),
		vy0 = if (@vy0 like '', NULL, @vy0),
		vz0 = if (@vz0 like '', NULL, @vz0),
		x0 = if (@x0 like '', NULL, @x0),
		y0 = if (@y0 like '', NULL, @y0),
