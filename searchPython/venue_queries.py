...
def most_games_played_at_venue(cursor,venue_name):
...
SELECT CONCAT(\"Most games have been played at \", venue_name, \" with total games played = \", gameCount) AS Answer FROM MaxGamesPlayed WHERE venue_name = \'{venue_name}\';'''
	sql = sql.format(venue_name = venue_name)
	cursor.execute(sql)
	return cursor.fetchall()

def total_games_per_venue(cursor,venue_name):
	sql = '''SELECT CONCAT(\"Total games played at \", venue_name, \" = \", count(venue_name)) AS Answer FROM Games WHERE venue_name = \'{venue_name}\';'''
	sql = sql.format(venue_name = venue_name)
	cursor.execute(sql)
	return cursor.fetchall()
